...
Code nodes can emit text, step, reasoning, tool-call, and activity lifecycles through the `Events.Add*` helper methods, including `AddStepStartAsync`, `AddStepEndAsync`, `AddReasoningMessageAsync`, `AddToolCallAsync`, `AddToolCallWithResultAsync`, `AddActivitySnapshotAsync`, `AddActivityDeltaAsync`, `AddActivitySnapshotFromContextAsync`, and `AddActivityDeltaFromContextAsync`.
