...
export class DesignerUpdateService {
  ...
  private getAddLocation(): [number, number] {
    const top =
      Math.round((Math.random() * 300) / DesignerUpdateService.GRID_SIZE) *
  ...
  addStartNode(workflow: WorkflowEntity) {
    ...
    workflow.nodes.update((nodes) => [
      ...nodes,
      StartNodeEntity.create(top, left),
    ]);
  }
  ...
  addEditNode(workflow: WorkflowEntity) {
    ...
    workflow.nodes.update((nodes) => [
      ...nodes,
      EditNodeEntity.create(top, left),
    ]);
  }
  ...
  addCodeNode(workflow: WorkflowEntity) {
    ...
    workflow.nodes.update((nodes) => [
      ...nodes,
      CodeNodeEntity.create(top, left),
    ]);
  }
  ...
  addSwitchNode(workflow: WorkflowEntity) {
    ...
    workflow.nodes.update((nodes) => [
      ...nodes,
      SwitchNodeEntity.create(top, left),
    ]);
  }
  ...
  addFanInNode(workflow: WorkflowEntity) {
    ...
    workflow.nodes.update((nodes) => [
      ...nodes,
      FanInNodeEntity.create(top, left),
    ]);
  }
  ...
  addFanOutNode(workflow: WorkflowEntity) {
    ...
    workflow.nodes.update((nodes) => [
      ...nodes,
      FanOutNodeEntity.create(top, left),
    ]);
  }
  ...
  addModelCallNode(workflow: WorkflowEntity) {
    ...
    workflow.nodes.update((nodes) => [
      ...nodes,
      ModelCallNodeEntity.create(top, left),
    ]);
  }
  ...
  addBatchNode(workflow: WorkflowEntity) {
    ...
    workflow.nodes.update((nodes) => [
      ...nodes,
      BatchNodeEntity.create(top, left),
    ]);
  }
  ...
  addGosubNode(workflow: WorkflowEntity) {
    ...
    workflow.nodes.update((nodes) => [
      ...nodes,
      GosubNodeEntity.create(top, left),
    ]);
  }
  ...
  addBackendToolCallNode(workflow: WorkflowEntity) {
    const [top, left] = this.getAddLocation();
    workflow.nodes.update((nodes) => [
      ...nodes,
  ...
  addStateSyncNode(workflow: WorkflowEntity) {
    const [top, left] = this.getAddLocation();
    workflow.nodes.update((nodes) => [
      ...nodes,
  ...
  addConnection(
    ...
    workflow.connections.update((connections) => [
      ...connections,
      ConnectionEntity.create(from.id, to.id),
    ]);
  }
  ...
  deleteNode(workflow: WorkflowEntity, node: NodeEntity<NodeSnapshot>) {
    ...
    workflow.connections.update((connections) =>
      connections.filter((c) => {
        ...
        if (
          fromConnector?.nodeId === node.id ||
          toConnector?.nodeId === node.id
        ) {
          entities.push(c);
          return false;
        }
  ...
  deleteConnection(workflow: WorkflowEntity, connection: ConnectionEntity) {
    workflow.connections.update((conns) =>
      conns.filter((c) => c.id !== connection.id),
    );
  }
  ...
  updateNodePositions(
    workflow: WorkflowEntity,
    updates: NodeEntity<NodeSnapshot>[],
    positions: Point[],
  ) {
    workflow.nodes.update((nodes) =>
      nodes.map((node) => {
  ...
  deleteSwitch(
    ...
    workflow.connections.update((conns) =>
      conns.filter((c) => c.from() !== connector.id),
    );
  }
