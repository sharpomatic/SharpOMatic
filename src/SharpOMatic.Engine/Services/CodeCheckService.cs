        ...
        if (!string.IsNullOrWhiteSpace(request.Code))
        {
            ...
            try
            {
                ...
                foreach (Diagnostic diagnostic in diagnostics.Where(d => d.Severity == DiagnosticSeverity.Error || d.Severity == DiagnosticSeverity.Warning))
                    results.Add(new CodeCheckResult(diagnostic.Severity, diagnostic.Location.SourceSpan.Start, diagnostic.Location.SourceSpan.End, diagnostic.Id, diagnostic.GetMessage()));
            }
