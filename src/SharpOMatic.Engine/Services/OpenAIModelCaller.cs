...
public class OpenAIModelCaller : BaseModelCaller
{
    ...
    )
    {
        ...
        (var chatOptions, var responseCreationOptions) = SetupResponsesBasicCapabilities(
            model,
            modelConfig,
            processContext,
            threadContext,
        ...
        var agentClient = GetOpenAIResponseClient(
            model,
            modelConfig,
        ...
        var agent = agentClient.CreateAIAgent(
    ...
    )
    {
        ...
        if (
            connectionFields.TryGetValue("organization_id", out var organizationId)
            && !string.IsNullOrWhiteSpace(organizationId)
        )
            options.OrganizationId = organizationId;

        if (
            connectionFields.TryGetValue("project_id", out var projectId)
    ...
    )
    {
        ...
        chatOptions =
            ...
            ?? new ChatOptions()
            {
                ...
                RawRepresentationFactory = (_) => responseOptions,
            };
    ...
    )
    {
        ...
        if (HasCapability(model, modelConfig, "SupportsToolCalling"))
        {
            ...
            agentServiceProvider = SetupToolCalling(
                chatOptions,
                model,
                modelConfig,
                processContext,
