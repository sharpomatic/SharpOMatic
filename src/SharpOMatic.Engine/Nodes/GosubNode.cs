    ...
    protected override async Task<(string, List<NextNodeData>)> RunInternal()
    {
        ...
        if (Node.ApplyInputMappings)
        {
            ...
            foreach (var entry in inputEntries)
            {
                ...
                if (!string.IsNullOrWhiteSpace(inputPath) && ThreadContext.NodeContext.TryGet<object?>(inputPath, out var mapValue))
                {
        ...
        var gosubContext = new GosubContext(ThreadContext.CurrentContext, Trace.TraceId, ThreadContext.NodeContext, returnNode, Node.ApplyOutputMappings, Node.OutputMappings);
