    ...
    protected override async Task<(string, List<NextNodeData>)> RunInternal()
    {
        ...
        if (Node.ApplyInputMappings)
        {
            ...
            foreach (var entry in inputEntries)
            {
                ...
                var targetPath = string.IsNullOrWhiteSpace(entry.OutputPath)
                    ? inputPath
                    : entry.OutputPath;
                ...
                )
                {
                    if (!childContext.TrySet(targetPath, mapValue))
                        throw new SharpOMaticException(
                            $"Gosub node input mapping could not set '{targetPath}' into context."
                        );
                }
                else
                {
                    if (!entry.Optional)
                    {
                        if (string.IsNullOrWhiteSpace(inputPath))
                            throw new SharpOMaticException(
                                "Gosub node input path cannot be empty."
                            );

                        throw new SharpOMaticException(
                            $"Gosub node mandatory path '{inputPath}' cannot be resolved."
                        );
                    }
        ...
        {
            var baseContextJson = ThreadContext.NodeContext.Serialize(
                ProcessContext.JsonConverters
            );
            childContext = ContextObject.Deserialize(
                baseContextJson,
                ProcessContext.JsonConverters
            );
        }

        var gosubContext = new GosubContext(
