    ...
    : RunNode<ModelCallNodeEntity>(threadContext, node)
{
    protected override async Task<(string, List<NextNodeData>)> RunInternal()
    {
        ...
        (var model, var modelConfig, var connector, var connectorConfig) =
            await LoadModelAndConnector();

        // Get the implementation for the specific connector config
        var caller = ProcessContext.ServiceScope.ServiceProvider.GetKeyedService<IModelCaller>(
            connectorConfig.ConfigId
        );
        if (caller is null)
            throw new SharpOMaticException(
                $"No implementation found for connector '{connectorConfig.ConfigId}'"
            );

        // Use specific implementation to perform call for us
        (var chat, var responses, var tempContext) = await caller.Call(
            model,
            modelConfig,
            connector,
            connectorConfig,
    ...
    private async Task<(Model, ModelConfig, Connector, ConnectorConfig)> LoadModelAndConnector()
    {
        ...
        var connector = await ProcessContext.RepositoryService.GetConnector(
            model.ConnectorId.Value,
            false
        );
        if (connector is null)
            throw new SharpOMaticException("Cannot find the model connector");

        var connectorConfig = await ProcessContext.RepositoryService.GetConnectorConfig(
            connector.ConfigId
        );
