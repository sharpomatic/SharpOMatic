...
- `selected_tools` entries are stored as `Class.Tool` so tools sharing a tool name can be told apart, but an unqualified entry is still valid and resolves to the first tool registered with that name. The model is always given the plain tool name, never the qualified one. `ToolSelectionHelper` owns the parsing and the per-tool setting lookups that tolerate both key forms.
